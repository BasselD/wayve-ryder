...
def find_4h_entry(zone, df_4h, earnings_cal=None, stats=None):
    """
    Within the daily W2 time/price window, find a 4H reversal confirmation.

    Earnings veto (2026-07-05): a confirmation bar with the ticker's earnings
    <= EARNINGS_VETO_DAYS away is skipped (continue, not break) — a later
    post-earnings confirmation in the same zone can still fire. Evidence:
    ...
    for ts in window_idx:
        ...
        # Earnings veto: skip confirmation bars inside the pre-earnings window.
        # continue (not break) so a post-earnings confirmation can still fire.
        if earnings_cal and earnings_veto(zone["ticker"], ts, earnings_cal):
            if stats is not None:
                stats["earnings_veto"] = stats.get("earnings_veto", 0) + 1
            continue
...
if __name__ == "__main__":
    ...
    print(f"   4H:    RSI<{FH_RSI_MAX} | MA{FH_TREND_MA} | Hold {FH_HOLD_BARS} bars (~{round(FH_HOLD_BARS*4/6.5)} trading days) | "
          f"R/R {MIN_RR}–{MAX_RR} | Earnings veto ≤{EARNINGS_VETO_DAYS}d\n")

    # Earnings calendar for the pre-earnings entry veto
    ...
    for ticker in TICKERS:
        try:
            ...
            veto_stats = {}
            for zone in zones:
                entry = find_4h_entry(zone, df_4h, earnings_cal, veto_stats)
            ...
            print(f"  [MTF-4H {ticker}] 4H entries found: {len(setups)} | No 4H match: {no_4h_entry} | "
                  f"Earnings-vetoed bars: {veto_stats.get('earnings_veto', 0)}")
