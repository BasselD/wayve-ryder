"""
APEX Backtest Engine v11
Changes from v10:
- SHORT: RSI filter at Wave B top now APPLIED — RSI_MIN_AT_WB_TOP (62) was defined but
  never enforced in scan_short_setups(). This was the primary cause of false signals.
- SHORT: RSI declining check added at confirmation bar — mirrors the long-side rising RSI
  check; ensures momentum is genuinely turning down, not just a one-bar spike reversal.
- SHORT: Dual-regime gate replaces stock-only MA200 check:
    SPY BEAR (below MA200) + stock below MA200 → Grade A, RSI_MIN_AT_WB_TOP (55)
    SPY BULL (above MA200) + stock ≥5% below MA200 → Grade B, RSI_BULL_WB_TOP (65)
    SPY BULL + stock <5% below MA200 OR stock above MA200 → NO TRADE
- SHORT: grade and spy_bear_regime fields added to setup output for post-analysis.
- LONG: zero changes — long scanner, simulation, and metrics are untouched.
"""
...
from events import load_earnings_calendar, earnings_veto, EARNINGS_VETO_DAYS
...
RSI_MIN_AT_WB_TOP = 55      # shorts BEAR regime: RSI at Wave B top must be > 55 (bounce above neutral)
RSI_BULL_WB_TOP   = 65      # shorts BULL regime: higher bar — bull bounces run hotter before exhausting
TREND_MA          = 50
...
def scan_setups(df, ticker, regime, earnings_cal=None):
    df = df.copy()
    ...
    for low_idx in swing_lows:
        ...
        for j in range(high_idx + 1, min(high_idx + 80, len(df))):
            ...
            # Earnings veto: no entries with earnings <= EARNINGS_VETO_DAYS away.
            # continue (not break) — a post-earnings confirmation can still fire.
            if earnings_cal and earnings_veto(ticker, entry_date, earnings_cal):
                skipped_earn += 1
                continue
    ...
    print(f"  [LONG  {ticker}] Filtered — W1 small: {skipped_w1_size} | Rule1: {skipped_rule1} | "
          ...
          f"Vol: {skipped_volume} | RSI: {skipped_rsi} | Regime: {skipped_regime} | "
          f"Earnings: {skipped_earn} | R/R: {skipped_rr}")
...
def scan_short_setups(df, ticker, spy_regime=None, earnings_cal=None):
    """
    Wave A-B-C correction scanner.
    A = significant swing high → swing low (downward impulse ≥ 7%)
    B = partial bounce 50–78.6% of Wave A, spanning ≥ 5 bars (genuine corrective structure)
    Entry = close below Wave B top bar's low (reversal confirmation)
    Stop  = above Wave A start (wave count invalidated if B exceeds A origin)
    T1    = projected DOWN from Wave B high at 1.0× Wave A length — exit 100%

    Dual-regime gate (v11):
      SPY BEAR (below MA200) + stock below MA200       → Grade A | RSI_MIN_AT_WB_TOP (55)
      SPY BULL (above MA200) + stock ≥5% below MA200  → Grade B | RSI_BULL_WB_TOP  (65)
      SPY BULL + stock <5% below MA200 OR above MA200 → NO TRADE

    RSI gate (v11 — previously missing):
      RSI at Wave B top bar must exceed the regime-specific threshold (62 or 68).
      RSI at confirmation bar must be FALLING vs the Wave B top bar.
      Together these confirm a genuine overbought trap reversing under momentum, not
      a weak bounce that was already exhausted before the entry fires.
    """
    df = df.copy()
    df["RSI"]   = calc_rsi(df["Close"], RSI_PERIOD)
    df["MA50"]  = df["Close"].rolling(TREND_MA).mean()
    ...
    for high_idx in swing_highs:
        ...
        low_idx  = next_lows[0]
        wa_start = df["High"].iloc[high_idx]
        wa_end   = df["Low"].iloc[low_idx]
        ...
        for j in range(low_idx + 1, min(low_idx + 80, len(df))):
            ...
            # ── Dual-regime gate ─────────────────────────────────────────────
            entry_date  = df.index[j]
            spy_is_bull = (spy_regime is not None
                           and entry_date in spy_regime.index
                           and bool(spy_regime.loc[entry_date]))

            ma200_at_entry = df["MA200"].iloc[j]
            stock_above_200 = (not pd.isna(ma200_at_entry)
                               and df["Close"].iloc[j] > ma200_at_entry)
            if stock_above_200:
                skipped_regime += 1
                continue

            if spy_is_bull:
                # Bull macro: only allow if stock is ≥5% below its MA200 (structural bear)
                if not pd.isna(ma200_at_entry):
                    dist_below = (ma200_at_entry - df["Close"].iloc[j]) / ma200_at_entry
                    if dist_below < 0.05:
                        skipped_regime += 1
                        continue
                grade = "B"
            ...
            # ── RSI gate (v11 — was entirely missing in v10) ─────────────────
            rsi_threshold = RSI_BULL_WB_TOP if spy_is_bull else RSI_MIN_AT_WB_TOP
            rsi_at_wb     = df["RSI"].iloc[wb_touch_bar]

            # Wave B RSI must be elevated — if not, the bounce was weak from the start
            if pd.isna(rsi_at_wb) or rsi_at_wb < rsi_threshold:
                skipped_rsi += 1
                break  # this WB zone is invalid regardless of confirmation bar timing

            # RSI must be falling at confirmation — momentum genuinely turning down
            rsi_at_confirm = df["RSI"].iloc[j]
            if pd.isna(rsi_at_confirm) or rsi_at_confirm >= rsi_at_wb:
                skipped_rsi += 1
                continue
            ...
            setups.append({
                ...
                "rsi_at_wb_top":    round(rsi_at_wb, 1),
                "rsi_at_confirm":   round(rsi_at_confirm, 1),
                "rr_ratio":         round(rr, 2),
                "grade":            grade,
                "spy_bear_regime":  not spy_is_bull,
                "entry_bar":        j,
            })
            break
    ...
    print(f"  [SHORT {ticker}] Filtered — WA small: {skipped_wa_size} | WB invalid: {skipped_wb_inv} | "
          f"Trend: {skipped_trend} | No confirm: {skipped_confirm} | "
          f"RSI: {skipped_rsi} | Regime: {skipped_regime} | Vol: {skipped_volume} | "
          f"Earnings: {skipped_earn} | R/R: {skipped_rr}")
...
if __name__ == "__main__":
    ...
    print(f"   RSI long<{RSI_MAX_AT_TOUCH} | RSI short (bear)>{RSI_MIN_AT_WB_TOP} | RSI short (bull)>{RSI_BULL_WB_TOP} | "
          f"Trend MA: {TREND_MA} | Regime: {REGIME_TICKER} {REGIME_MA}MA | Earnings veto ≤{EARNINGS_VETO_DAYS}d\n")
    ...
    # Short scanner needs the real SPY regime (not QQQ fallback).
    # Load SPY independently so the long-side regime is unaffected.
    try:
        spy_short_df = load_data("SPY")
        spy_short_df["MA200"] = spy_short_df["Close"].rolling(REGIME_MA).mean()
        spy_short_regime_full = spy_short_df["Close"] > spy_short_df["MA200"]
        spy_short_loaded      = True
    except Exception:
        spy_short_regime_full = spy_regime_full  # fall back to long-side regime if SPY also missing
        spy_short_loaded      = spy_loaded

    # Earnings calendar for the pre-earnings entry veto (longs + shorts)
    ...
    for ticker in TICKERS:
        try:
            ...
            # ── LONGS ──
            long_setups  = scan_setups(df, ticker, regime, earnings_cal=earnings_cal)
            ...
            # ── SHORTS ──
            spy_short_regime = (spy_short_regime_full.reindex(df.index, method="ffill").fillna(False)
                                if spy_short_loaded else None)
            short_setups = scan_short_setups(df, ticker, spy_regime=spy_short_regime, earnings_cal=earnings_cal)
